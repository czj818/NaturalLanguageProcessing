...
class LdaTopicCounts:
    ...
    def report(self, vocab, handle, limit=50):
        ...
        for kk in self._normalizer:
            ...
            word = 0
            #word_prob = [self.word_in_topic(kk, ww) for ww in self._topic_term[kk]]
            #print (word_prob)
            #new_dict = zip(self._topic_term[kk], word_prob)
            #dic = {self._topic_term[kk][i]: word_prob[i] for i in range(len(word_prob))}          
            #sorted_word = [key for key,value in sorted(dic.items(),key=lambda item: item[1], reverse = True)]
            lst = self._topic_term[kk].most_common(50)
            wrd_lst =[i[0] for i in lst]
            for ww in wrd_lst:
...
class Sampler:
    ...
    def run_sampler(self, iterations = 1000):
        """
    ...
    def report_topics(self, vocab, outputfilename, limit=50):
        ...
        #tmp = "w\tlikelihood\ttime(s)\n"
        #for t in range(0, len(self._num_topics)):
         #   self._topic_lhood.sort(reverse=True)
          #  for wd in range(0,limit):
           #     tmp = str(wd) + "\t" + str(self._topic_lhood[wd]) + "\t" + str(self._time[wd]) + "\n"
            #    topicsfile.write(tmp)
        #topicsfile.close()
    ...
    def sample_probs(self, doc_id, index):
        ...
        for kk in range(self._num_topics):
            ...
            # uniform probability.
            term_kk = self._topics.word_in_topic(kk,term)
            doc_topic = float(self._doc_counts[doc_id][kk] + self._alpha[kk]) /             float(sum(self._doc_counts[doc_id].values()) + sum(self._alpha))
            sample_probs[kk] = term_kk * doc_topic
...
topic_term = defaultdict(FreqDist)
