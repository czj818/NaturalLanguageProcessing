...
def train(save_dir='saved_weights',
    ...
    # Look in the ModelConfig class for the model's hyperparameters
    config = dataset.model_config

    print('Loading embeddings')
    word_embeddings, pos_embeddings, dep_embeddings = load_embeddings(config)
    # For Task 3, add Twitter and Wikipedia embeddings (do this last)
    ...
    #loss_fn = criterion(output,label)
    ...
    #optimizer = optim.SGD(parser.parameters(), lr = config.lr)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i, (train_x, train_y) in enumerate(get_minibatches([dataset.train_inputs,
            ...
            pos_inputs_batch = torch.LongTensor(pos_inputs_batch)
            dep_inputs_batch = torch.LongTensor(dep_inputs_batch)
            
            word_inputs_batch.to(device)
            pos_inputs_batch.to(device)  # TODO 
            ...
            # function in numpy.
            labels = [np.argmax(x) for x in train_y] # TODO 
            ...
            if i % print_every_iters == 0:
                print ('Epoch: %d [%d], loss: %1.3f, acc: %1.3f'                        % (epoch, i, loss.item(),
